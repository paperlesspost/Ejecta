...
- (void)didConnect
{
    ...
    int __unused retVal = SecRandomCopyBytes(kSecRandomDefault, keyBytes.length, keyBytes.mutableBytes);
...
- (void)_sendFrameWithOpcode:(SROpCode)opcode data:(id)data;
{
    ...
    if (!useMask) {
        ...
        int __unused retVal = SecRandomCopyBytes(kSecRandomDefault, sizeof(uint32_t), (uint8_t *)mask_key);
